<template>
  <div>
    ...
    <div class="screen" :style="screenStyle" @click.self="handleActivated(-1)" ref="screen">
      <div class="component" v-for="(item, index) in chartData.elements" :key="index"
        :style="{width: item.w + 'px', height: item.h + 'px', left: item.x + 'px', top: item.y + 'px', zIndex: chartData.elements.length - index}">
        <div class="filter" v-if="item.data.type === 'chart'"
          ...
          <ve-liquidfill v-else-if="item.data.settings.type=='liquidfill'" :width="item.w + 'px'"
            :height="item.h + 'px'" :data="item.data.generated" @ready-once="generateData(item)" />
          <ve-chart v-else :width="item.w + 'px'" :height="item.h + 'px'" :data="item.data.generated"
            :settings="item.data.settings" @ready-once="generateData(item)" />
        </div>
        <div class="filter" v-if="item.data.type == 'text'"
          :style="{width: '100%', height: '100%', backgroundColor: item.bgcolor}">
        ...
        <div class="filter" v-if="item.data.type == 'image'"
          :style="{width: '100%', height: '100%', backgroundColor: item.bgcolor}">
          <div class="imagecontainer" :style="{backgroundImage: `url(${item.data.datacon.img})`, 
