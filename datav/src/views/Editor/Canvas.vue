<template>
  <div>
    <el-dialog title="发布" :visible.sync="$parent.publishPopVisible" width="50%">
      ...
      <el-input v-model="publicUrl" readonly="readonly"></el-input><span slot="footer">
        <el-button type="primary" @click="$parent.publishPopVisible = false">确 定</el-button>
      </span>
    </el-dialog>
    <div class="edit-view" tabindex="0" @keydown.space.prevent="handleSpaceDown" @keyup.space.prevent="handleSpaceUp"
      @click.self="handleActivated(-1)">
      <vue-draggable-resizable :style="wrapStyle" :x="100" :y="50" :w="chartData.w" :h="chartData.h"
        class-name="screen-box" class-name-draggable="screen-box-draggable" :draggable="screenDraggable"
        :resizable="false">
        <div class="screen" :style="screenStyle" @click.self="handleActivated(-1)" ref="screen">
          <vue-drag-resize v-for="(item, index) in chartData.elements" :key="index"
            :isActive="item.active &amp;&amp; !$parent.preview" :parentScaleX="scale" :parentScaleY="scale" :x="item.x"
            :y="item.y" :w="item.w" :h="item.h" :parentLimitation="true" :parentW="chartData.w" :parentH="chartData.h"
            :aspectRatio="false" :minw="20" :minh="20" :z="chartData.elements.length - index"
            :isDraggable="!$parent.preview" :isResizable="!$parent.preview" @activated="handleActivated(index)"
            @resizing="handleResize(item, arguments[0])" @dragging="handleDrag(item, arguments[0])">
            <div class="filler" v-if="item.data.type == 'chart'"
              :style="{width: '100%', height: '100%', backgroundColor: item.bgcolor}">
              <ve-map v-if="item.data.settings.type=='map'" :width="item.w + 'px'" :height="item.h + 'px'"
                :data="item.data.generated" :settings="item.data.settings" @ready-once="generateData(item)"></ve-map>
              <ve-liquidfill v-else-if="item.data.settings.type=='liquidfill'" :width="item.w + 'px'"
                :height="item.h + 'px'" :data="item.data.generated" @ready-once="generateData(item)"></ve-liquidfill>
              <ve-chart v-else="v-else" :width="item.w + 'px'" :height="item.h + 'px'" :data="item.data.generated"
                :settings="item.data.settings" @ready-once="generateData(item)"></ve-chart>
            </div>
            <div class="filler" v-if="item.data.type == 'text'"
              ...
              <div class="textcontainer"
                ...
                v-text="item.data.datacon.text"></div>
            </div>
            ...
            <div class="filler" v-if="item.data.type == 'border'"
              ...
              <div class="bordercontainer" :class="'border' + item.data.datacon.borderId"
                :style="{opacity: item.data.datacon.opacity}"></div>
            </div>
          </vue-drag-resize>
