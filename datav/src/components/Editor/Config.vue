    ...
    <div class="public-config" v-if="!currentElement.w">
      ...
      <div class="config-box">
        ...
        <el-row :gutter="20" style="margin-top: 12px;" v-show="editorSettings.parentBg === 1">
          <el-col :span="24">
            <el-upload class="bg-uploader" action="http://localhost:3000/api/uploadfile/" :show-file-list="false"
              :on-success="handleScreenBgUploadSuccess" :before-upload="beforeUpload">
          ...
          <el-col :span="24" v-show="chartData.bgimage" style="margin-top: 16px">
            <el-button type="danger" plain="plain" @click="handleScreenBgDelete" style="width: 100%">删除</el-button>
          </el-col>
        </el-row>
      </div>
    </div>
    <div class="component-config" v-if="currentElement.w">
      <div class="panel-selector">
        <div class="radio-group">
          <div class="radio-btn" @click="thisKey='general'" :class="{active: thisKey=='general'}">基础</div>
          <div class="radio-btn" @click="thisKey='data'" v-show="currentElement.data.type == 'chart'"
            :class="{active: thisKey=='data'}">数据</div>
          <div class="radio-btn" @click="thisKey='data'" v-show="currentElement.data.type == 'text'"
            :class="{active: thisKey=='data'}">文字</div>
          <div class="radio-btn" @click="thisKey='data'" v-show="currentElement.data.type == 'image'"
            :class="{active: thisKey=='data'}">图片</div>
          <div class="radio-btn" @click="thisKey='data'" v-show="currentElement.data.type == 'border'"
            :class="{active: thisKey=='data'}">边框</div>
        </div>
      </div>
      ...
      <div class="panel" v-show="thisKey=='data' &amp;&amp; currentElement.data.type == 'chart'">
        <div class="config-box">
          ...
          <el-select v-model="currentElement.data.datacon.type" placeholder="请选择" @change="handleChartDataChange"
          ...
          <vue-json-editor v-if="currentElement.data.datacon.type == 'raw'" v-model="currentElement.data.datacon.data"
            mode="code" :show-btns="true" @json-save="handleChartDataChange"></vue-json-editor>
          <el-select v-if="currentElement.data.datacon.type == 'connect'"
            v-model="currentElement.data.datacon.connectId" placeholder="请选择" @change="handleChartDataChange"
            style="width: 100%; margin-bottom: 10px;">
            <el-option v-for="item in connectList" :label="item.name" :key="item._id" :value="item._id"></el-option>
          </el-select>
          <el-input v-if="currentElement.data.datacon.type == 'get'" v-model="currentElement.data.datacon.getUrl"
            type="textarea" :rows="5" style="margin-bottom: 10px;"></el-input>
          <el-row v-if="currentElement.data.datacon.type == 'get'">
            <el-col :span="8">
              <p style="margin-top: 8px;">刷新时间</p>
            </el-col>
            <el-col :span="16">
              <el-input-number v-model="currentElement.data.datacon.interval" :min="1" :max="10"
                @change="handleChartDataChange" style="width: 100%;"></el-input-number>
            </el-col>
          </el-row>
        </div>
      </div>
      <div class="panel" v-show="thisKey=='data' &amp;&amp; currentElement.data.type == 'text'">
        <div class="config-box">
          ...
          <el-input v-model="currentElement.data.datacon.text" type="textarea" :rows="5" style="margin-bottom: 10px;">
          </el-input>
        </div>
        <div class="config-box">
          <div class="title">字体字号</div>
          <el-select v-model="currentElement.data.datacon.fontFamily" placeholder="请选择"
            style="width: 100%; margin-bottom: 10px;">
            <el-option-group label="英文字体">
              <el-option label="Molengo" value="Molengo"><span :style="{fontFamily: 'Molengo'}">Molengo</span>
              </el-option>
              <el-option label="Lobster" value="Lobster"><span :style="{fontFamily: 'Lobster'}">Lobster</span>
              </el-option>
            </el-option-group>
            <el-option-group label="中文字体">
              <el-option label="思源黑体" value="Noto Sans SC"><span :style="{fontFamily: 'Noto Sans SC'}">思源黑体</span>
              </el-option>
              <el-option label="思源宋体" value="Noto Serif SC"><span :style="{fontFamily: 'Noto Serif SC'}">思源宋体</span>
              </el-option>
              <el-option label="站酷庆科黄油体" value="ZCOOL QingKe HuangYou"><span
                  :style="{fontFamily: 'ZCOOL QingKe HuangYou'}">站酷庆科黄油体</span></el-option>
              <el-option label="站酷小薇体" value="ZCOOL XiaoWei"><span :style="{fontFamily: 'ZCOOL XiaoWei'}">站酷小薇体</span>
              </el-option>
            </el-option-group>
          </el-select>
          <el-row :gutter="20">
          ...
          <el-row :gutter="20">
            <el-col :span="24">
              <div class="btn" :class="{active: currentElement.data.datacon.bold}"
                @click="currentElement.data.datacon.bold = !currentElement.data.datacon.bold"><i
                  class="iconfont icon-bold"></i></div>
              <div class="btn" :class="{active: currentElement.data.datacon.italic}"
                @click="currentElement.data.datacon.italic = !currentElement.data.datacon.italic"><i
                  class="iconfont icon-italic"></i></div>
            </el-col>
          </el-row>
        </div>
        <div class="config-box">
      ...
      <div class="panel" v-show="thisKey=='data' &amp;&amp; currentElement.data.type == 'image'">
        <div class="config-box">
          ...
          <el-upload class="bg-uploader" action="http://localhost:3000/api/uploadfile/" :show-file-list="false"
            :on-success="handleImageUploadSuccess" :before-upload="beforeUpload">
            <div class="bg-preview-wrapper" v-if="this.currentElement.data.datacon.img"><img class="bg-preview"
                :src="this.currentElement.data.datacon.img" /></div><i class="el-icon-plus avatar-uploader-icon"
              v-else></i>
          </el-upload>
          <el-row>
        ...
        <div class="config-box">
          ...
          <el-slider v-model="currentElement.data.datacon.opacity" :max="1" :step="0.01" show-input="show-input">
          </el-slider>
        </div>
      </div>
      <div class="panel" v-show="thisKey=='data' &amp;&amp; currentElement.data.type == 'border'">
        <div class="config-box">
          ...
          <el-select v-model="currentElement.data.datacon.borderId" placeholder="请选择"
            style="width: 100%; margin-bottom: 10px;">
